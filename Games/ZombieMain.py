...
health.position = 100, 300
health_group.add(health)

#create heath sprite
health2 = MySprite()
health2.load("health.png", 32, 32, 1)
health2.position = 400,300
health_group.add(health2)

crop = MySprite()
crop.load("crop.png", 50, 66, 1)
crop.position = 600, 300
crop_group.add(crop)
...
while True:
    ...
    if not game_over:
        ...
        for z in zombie_group:
            ...
            z.velocity.x *= random.randint(0,6)
            z.velocity.y *= random.randint(0,6)
        ...
        zcollide = None
        zcollide = pygame.sprite.groupcollide(zombie_group, zombie_group, False, False)
        if zcollide != None:
            z.X + 1
        else:
            zcollide = None
        ...
        if pygame.sprite.collide_rect_ratio(0.5)(player,health2):
            player_health += 30
            if player_health > 100: player_health = 100
            health2.X = random.randint(0,700)
            health2.Y = random.randint(0,500)

        if pygame.sprite.collide_rect_ratio(0.5)(player,crop):
            score += 10
            crop.X = random.randint(0,700)
            crop.Y = random.randint(0,500)
    ...
    #clear the screen
    nuke = pygame.image.load("nuke.jpg")
    ...
    print_text(font, 300, 550, "SCORE: " + str(score))

    if game_over:
        print_text(font, 300, 100, "G A M E   O V E R")
        print_text(font, 300, 200, "YOUR SCORE:  " + str(score))
